...
| `proof_request_validate` | `fuzz/fuzz_targets/proof_request_validate.rs` | `src/verifier.rs`: `ProofVerificationRequest` and its inherent `validate()` method | JSON deserialization followed by fail-closed contract and policy validation for successfully decoded requests; this is **not** cryptographic BitVM2 proof verification |
