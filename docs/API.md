...
High-privilege endpoints (Proposal Approval, Execution, MCP surface, State Commit, ERP Sync, and Lightning Payments) require administrative authentication via the `X-Gateway-Admin-Key` header. The expected value must be configured in the `GATEWAY_ADMIN_API_KEY` environment variable. If the environment variable is not set, these endpoints will return `503 Service Unavailable`.
