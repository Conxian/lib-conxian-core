...
9. **DLC**: Equation verification is retained, intent-bound policy checks are typed, `DlcIntent::validate` enforces fail-closed secp256k1 key and parameter invariants, and CET structure/payout validation is hardened (`DlcManager::validate_cet_structure`); execution, funding, and finality remain downstream (CON-1509).
