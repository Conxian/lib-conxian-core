...
> **Session 52 end-to-end Nitro provisioning (2026-08-05):** Pipeline now fully operational: build-test → docker-build → provision-nitro → teardown. 7 spot instance types tried (fallback chain), c5.xlarge used. SG `conxian-nitro-sg`, instance profile `conxian-nitro-enclave-profile`, role `conxian-nitro-enclave-role` all created. Dynamic AMI/subnet discovery. IAM policy attached to `botshelo`. Main CI + SDK compat workflows bumped to rustc 1.97.
