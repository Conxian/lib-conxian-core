...
This directory contains bounded `cargo-fuzz` regression targets for current
`lib-conxian-core` APIs and selected dependency-level parser/aggregation
surfaces. The targets are intended to find parser panics, unexpected aborts,
and dependency regressions; malformed input and a normal `false` result are
expected for the validation targets.
...
aggregator. Dependency code may allocate internally after receiving this
...
Vault signing, MuSig2 sessions, attestation, and BitVM2 proof verification
belong to [`conxius-enclave-sdk`](https://crates.io/crates/conxius-enclave-sdk),
not this crate.
...
Run one target for up to ten seconds with a five-second per-input timeout and a
2 GiB RSS ceiling. This is the CI-equivalent command and keeps generated input
length bounded at 16 KiB:

```bash
cargo +nightly fuzz run anchoring_receipt fuzz/corpus/anchoring_receipt -- \
  -max_total_time=10 -timeout=5 -rss_limit_mb=2048 -max_len=16384
```

On a small development machine, use the unoptimized, unsanitized mode to keep
the Rust dependency build below the available memory ceiling:

```bash
cargo +nightly fuzz run --sanitizer none --dev --jobs 1 \
  anchoring_receipt fuzz/corpus/anchoring_receipt -- \
  -max_total_time=5 -timeout=2 -rss_limit_mb=2048 -max_len=16384
```

Run the bounded local smoke suite for all current targets in that constrained
mode:
...
`-max_total_time=10` with `-max_total_time=30`.
...
fails on fuzz-run errors, and uploads target-specific corpus/artifact output
when a job fails.
