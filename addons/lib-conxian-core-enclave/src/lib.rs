...
pub const SUPPORTED_SDK_VERSION: &str = "2.0.17";
...
/// Computes the domain-separated digest that is sent to SDK `2.0.17`.
///
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedDigestAlgorithm(algorithm) => {
                write!(
                    formatter,
                    "digest algorithm {algorithm:?} is unsupported by SDK 2.0.17"
                )
            }
            ...
            Self::UnsupportedPublicKeyDerivation(algorithm) => write!(
                formatter,
                "public-key derivation is unsupported for {algorithm:?} with SDK 2.0.17"
            ),
...
/// Converts a Core algorithm to the exact SDK `2.0.17` enum.
...
/// Converts an exact SDK `2.0.17` algorithm to the Core enum.
...
/// Returns whether the adapter's conservative SDK `2.0.17` capability
...
///
/// SDK `2.0.17` exposes only `message_hash: Vec<u8>`, so the adapter supports
...
/// Injected-manager adapter over the exact SDK `2.0.17` `EnclaveManager`
/// interface.
